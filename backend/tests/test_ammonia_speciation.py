...
Skrip assert stdlib, bukan test pytest, karena `pytest` tidak ada di
backend/requirements.txt. Jalankan langsung:

    docker compose exec backend python tests/test_ammonia_speciation.py

Namanya tetap `test_*` supaya ikut terjaring kalau pytest dipasang nanti.
"""
...
def test_kondisi_khas_kolam():
    ...
    Menyimpang dari amonia.md:147 yang mengharapkan PERHATIAN. classify_risk()
    di dokumen yang sama (amonia.md:120) menggolongkan < 6.0 % sebagai NORMAL,
    dan fraksinya 5.9985 %, tepat di bawah ambang. Keputusan 2026-09-03:
    ikuti ambang 6 %/15 %, jadi yang benar NORMAL.
    """
...
def test_di_luar_rentang_tetap_dihitung():
    """pH 7.0 di luar rentang tervalidasi 7.8-8.3: ditandai, bukan diblokir."""
